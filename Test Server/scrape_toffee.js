...
async function scrape() {
    const url = "https://toffee.allinonereborn.workers.dev/?token=MTc4MzQ0NjQwMTY1Mw==.m3u";
    console.log("Fetching Toffee playlist from URL...");
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('#EXTINF:')) {
            ...
            if (lastCommaIdx !== -1) {
                ...
                currentChannel.id = slugify(currentChannel.name) + '-toffee';
            }
            ...
            if (headersPart) {
                ...
                for (const pair of pairs) {
                    ...
                    if (eqIndex !== -1) {
                        ...
                        if (key && val) {
                            // Map user-agent to User-Agent
                            const headerKey = key.toLowerCase() === 'user-agent' ? 'User-Agent' : key;
    ...
    console.log(`Parsed ${channels.length} valid channels from Toffee M3U.`);
    ...
    channelsJson.categories['toffee'] = {
        name: "Toffee",
    ...
    let sqlContent = `-- SQL Script to insert/upsert Toffee channels and playlist\n`;
    
    // Category Insert
    sqlContent += `INSERT INTO public.categories (id, name, sort_order) \n`;
    sqlContent += `VALUES ('toffee', 'Toffee', 11) \n`;
    ...
    for (const item of uniquePayload) {
        ...
        sqlValues.push(`('${idEscaped}', '${nameEscaped}', ${logoEscaped}, 'toffee', true, 'HD', '${streamUrlEscaped}', '${headersStr}'::jsonb, ${proxyVal}, ${drmVal}::jsonb)`);
    }
    ...
    sqlContent += `VALUES ('toffee', 'Toffee', ARRAY[${channelIdsArrayStr}]) \n`;
    ...
    fs.writeFileSync('insert_toffee_channels.sql', sqlContent, 'utf8');
    console.log("SQL insert script written to insert_toffee_channels.sql successfully.");
}
